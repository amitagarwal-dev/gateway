...
const packager  = require ('./lib/packager/FINO').packager;
...
function FrameFINOHeader (ISOMessageBuffer) {
	let vendorID 	= '00' + TransXpedia.CLIENT_JSON.VENDORID;
	vendorID 		= vendorID.slice(-2);
	let header 		= `${vendorID}00110011ISO026000010`;
	
	ISOMessageBuffer[0] = ((ISOMessageBuffer.length-2) >> 8) & 0xff;//previously we are adding two bytes of package_length indicator in 
									//overall package_length of isobuffer
	ISOMessageBuffer[1] = (ISOMessageBuffer.length-2) & 0xff;

	for (let i = 0; i < header.length; i++) {
		ISOMessageBuffer[i+2] = header[i].charCodeAt();
	}
